...
class ReactDocGenMarkdownRenderer {
  ...
  compile({ template, plugins = [] } = {}) {
    this.template = template || this.options.template;
    this.template.setPlugins(plugins, this.extension);
  }

  render(file, docs, composes) {
    return this.template.instantiate({
      componentName: docs.displayName ? docs.displayName : path.basename(file, path.extname(file)),
