...
module.exports = {
  simpleComponent: ({
    ...
    ${componentName}.propTypes = {
      ${composes.map(compose => `...${compose}.propTypes`)}${
      composes.length > 0 ? ',' : ''
    }${props.map(
      prop => `${
        prop.description
      ...
      */ `
      ...
      ${prop.name}: ${!prop.custom ? 'PropTypes.' : ''}${prop.type}${
...
${types
  .map(
    type =>
      `**${type.name}** | \`${type.value}\` | ${type.default ? `\`${type.default}\`` : ''} | ${
        type.required ? ':white_check_mark:' : ':x:'
...
${
  composes.length > 0
    ? `${os.EOL}${componentName} gets more \`propTypes\` from these composed components
${composes
  .map(
    compose => `#### ${compose.name}

${
  compose.types.length > 0
    ? `prop | type | default | required | description
---- | :----: | :-------: | :--------: | -----------
${compose.types
  .map(
