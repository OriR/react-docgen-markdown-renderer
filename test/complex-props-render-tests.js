...
lab.experiment('complex render', () => {
  ...
  lab.experiment('union', () => {
    lab.beforeEach(({ context }) => {
      context.getUnionProp = types => ({
        name: 'unionProp',
        type: `oneOfType([${types.map(type => `PropTypes.${type}`)}])`,
      });
    });
      ...
      const result = context.renderer.render(
        ...
        reactDocgen.parse(
          simpleComponent({
            componentName: 'MyComponent',
            props: [
              context.getUnionProp(['string', 'oneOfType([PropTypes.string, PropTypes.number])']),
            ],
          }),
        ),
        [],
  ...
  lab.experiment('arrayOf', () => {
    ...
    lab.test('with arrayOf', ({ context }) => {
      ...
      expect(result).to.equal(
        simpleMarkdown({ types: [{ name: 'arrayOfProp', value: 'Array[]<Array[]<Number>>' }] }),
      );
    });
    ...
    lab.test('with objectOf', ({ context }) => {
      ...
      expect(result).to.equal(
        simpleMarkdown({ types: [{ name: 'arrayOfProp', value: 'Array[]<Object[#]<Number>>' }] }),
      );
    });
  ...
  lab.experiment('objectOf', () => {
    ...
    lab.test('with arrayOf', ({ context }) => {
      ...
      expect(result).to.equal(
        simpleMarkdown({ types: [{ name: 'objectOfProp', value: 'Object[#]<Array[]<Number>>' }] }),
      );
    });
    ...
    lab.test('with objectOf', ({ context }) => {
      ...
      expect(result).to.equal(
        simpleMarkdown({
          types: [{ name: 'objectOfProp', value: 'Object[#]<Object[#]<Number>>' }],
        }),
      );
    });
  ...
  lab.experiment('shape', () => {
    lab.beforeEach(({ context }) => {
      context.getShapeProp = types => ({
        name: 'shapeProp',
        type: `shape({ ${types.map(type => `${type.name}: PropTypes.${type.type}`)} })`,
      });
    });
        ...
        reactDocgen.parse(
          simpleComponent({
            ...
            props: [
              context.getShapeProp([
                { name: 'stringProp', type: 'string' },
                { name: 'objectOfProp', type: 'objectOf(PropTypes.number)' },
              ]),
            ],
          }),
        ),
        ...
        reactDocgen.parse(
          simpleComponent({
            ...
            props: [
              context.getShapeProp([
                { name: 'stringProp', type: 'string' },
                { name: 'shapeProp', type: 'shape({ index: PropTypes.number })' },
              ]),
            ],
          }),
        ),
        ...
        reactDocgen.parse(
          simpleComponent({
            ...
            props: [
              context.getShapeProp([
                { name: 'stringProp', type: 'string' },
                { name: 'unionProp', type: 'oneOfType([PropTypes.string, PropTypes.number])' },
              ]),
            ],
          }),
        ),
