...
lab.experiment('prop metadata render', () => {
          ...
          props: [
            {
              name: 'stringProp',
              type: 'string',
              description: 'This is a custom required string prop with default',
            },
          ],
        }),
      ),
  ...
  lab.test('required', ({ context }) => {
    ...
    expect(result).to.equal(
      simpleMarkdown({ types: [{ name: 'stringProp', value: 'String', required: true }] }),
    );
  });
  ...
  lab.test('default', ({ context }) => {
    ...
    expect(result).to.equal(
      simpleMarkdown({ types: [{ name: 'stringProp', value: 'String', default: "'value'" }] }),
    );
  });
