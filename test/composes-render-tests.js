...
lab.experiment('compose render', () => {
  ...
  lab.test('simple', ({ context }) => {
    ...
    expect(result).to.equal(
      simpleMarkdown({
        types: [{ name: 'stringProp', value: 'String' }],
        composes: [{ name: 'AnotherComponent', types: [{ name: 'numberProp', value: 'Number' }] }],
      }),
    );
  });
  ...
  lab.test('without props', ({ context }) => {
    ...
    expect(result).to.equal(
      simpleMarkdown({
        types: [{ name: 'stringProp', value: 'String' }],
        composes: [{ name: 'AnotherComponent', types: [] }],
      }),
    );
  });
  ...
  lab.test('multi', ({ context }) => {
    ...
    expect(result).to.equal(
      simpleMarkdown({
        types: [{ name: 'stringProp', value: 'String' }],
        composes: [
          { name: 'AnotherComponent', types: [{ name: 'numberProp', value: 'Number' }] },
          { name: 'AndAnother', types: [{ name: 'boolProp', value: 'Boolean' }] },
        ],
      }),
    );
  });
  ...
  lab.test('missing', ({ context }) => {
    ...
    expect(result).to.equal(
      simpleMarkdown({ types: [{ name: 'stringProp', value: 'String' }], isMissing: true }),
    );
  });
  ...
  lab.test('imported types', ({ context }) => {
    ...
    expect(result).to.equal(
      simpleMarkdown({ types: [{ name: 'customImportedShape', value: 'customImportedShape' }] }),
    );
  });
    ...
    const result = context.renderer.render(
      ...
      reactDocgen.parse(
        simpleComponent({
          ...
          props: [
            {
              name: 'nestedImportedShape',
              custom: true,
              type:
                'PropTypes.oneOfType([PropTypes.arrayOf(customImportedShape), customImportedShape])',
            },
          ],
        }),
      ),
    ...
    expect(result).to.equal(
      simpleMarkdown({
        types: [
          {
            name: 'nestedImportedShape',
            value: 'Union<Array[]<customImportedShape>\\|customImportedShape>',
          },
  ...
  lab.test('custom componentName', ({ context }) => {
    const anotherComponent = reactDocgen.parse(
      simpleComponent({
        componentName: 'AnotherComponent',
      }),
    );
    ...
    expect(result).to.equal(
      simpleMarkdown({
        types: [{ name: 'stringProp', value: 'String' }],
        composes: [{ name: 'CustomComponentName', types: [] }],
      }),
    );
  });
});
